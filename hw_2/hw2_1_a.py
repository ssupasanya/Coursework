...
# Add record number
for record, expense in enumerate(expenses, 1):
    print(f"{record:4d}:{expense}")
    separated_expenses.append(expense.split(':'))

# Add formatting
for record, expense in enumerate(separated_expenses, 1):
    print(f"{record:4d} {expense[0]:>8s} {expense[1]:>10s} {expense[2]:10s} {expense[3]:s}")
